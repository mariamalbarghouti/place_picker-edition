...
class PlacePickerState extends State<PlacePicker> {
    ...
    return WillPopScope(
      onWillPop: () async {
        if (locationResult != null) {
          Get.back(result: {"location": locationResult});
          return true;
        } else {
          print("locationResult locationResult $locationResult");
          return false;
        }
      },
      child: Scaffold(
  ...
  Future<void> searchPlace(String place) async {
    ...
    await moveToLocation(locationResult?.latLng ?? LatLng(5.6037, 0.1870));
    clearOverlay();
    setState(() {
  ...
  Future<void> autoCompleteSearch(String place) async {
    try {
      ...
      var endpoint =
          ...
          "key=${widget.apiKey}&"
          "language=${widget.localizationItem?.languageCode??"en_us"}&"
      ...
      if (predictions.isEmpty) {
        ...
        for (dynamic t in predictions) {
          ...
          suggestions.add(RichSuggestion(aci, () async {
            FocusScope.of(context).requestFocus(FocusNode());
            await decodeAndSelectPlace(aci.id);
          }));
        }
      }
  ...
  Future<void> moveToLocation(LatLng latLng) async {
    // latLong = latLng;
    this.mapController.future.then((controller) async {
